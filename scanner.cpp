...
vector<string> reserved_words = {
    ...
    "if",      "int",    "length", "main",    "new",
    "public",  "return", "static", "String",  "System.out.println",
  ...
  while (true) {
    ...
    case 26:
      if (!isalnum(input[pos]) && input[pos] != '_') {
        if (strcmp(lexeme.c_str(), "System") == 0 ||
            strcmp(lexeme.c_str(), "System.out") == 0) {
          lexeme += input[pos];
      ...
      break;
    case 27:
      if (find(reserved_words.begin(), reserved_words.end(), lexeme) !=
          reserved_words.end()) {
        if (strcmp(lexeme.c_str(), "System.out.println") == 0) {
          tok = new Token(SOUT);
